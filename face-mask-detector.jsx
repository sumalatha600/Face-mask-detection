        ...
        <div className="main">
          ...
          <div className="left-panel">
            ...
            {results?.overall_assessment && (
              <div style={{ background: "var(--panel)", border: "1px solid var(--border2)", padding: "14px 16px" }}>
                <div style={{ fontFamily: "var(--mono)", fontSize: "0.6rem", color: "var(--cyan-dim)", letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: 8 }}>AI Assessment</div>
                <div style={{ fontFamily: "var(--mono)", fontSize: "0.74rem", color: "var(--text)", lineHeight: 1.7 }}>{results.overall_assessment}</div>
                {results.scene_description && (
                  <div style={{ fontFamily: "var(--mono)", fontSize: "0.65rem", color: "var(--muted)", marginTop: 8 }}>Scene: {results.scene_description}</div>
                )}
              </div>
            )}

            {/* How it works — algo cards */}
            <div>
              ...
              <div className="algo-cards">
                {[
                  { title: "🖼 Image Preprocessing", desc: "Image is base64-encoded and sent to the vision model. Resolution and lighting normalization is handled by the model." },
                  { title: "👁 Face Detection", desc: "Claude Vision identifies all human faces in the scene, regardless of angle, lighting, or occlusion." },
                  { title: "😷 Mask Classification", desc: "Each detected face is classified: MASKED (full coverage), UNMASKED (no mask), or PARTIAL (improper wear)." },
                  { title: "📊 Compliance Scoring", desc: "Compliance rate = (masked faces / total faces) × 100. Used for safety monitoring and reporting." },
                ].map(a => (
                  <div key={a.title} className="algo-card">
                    <div className="algo-title">{a.title}</div>
            ...
            <div className="right-section">
              ...
              <div className="stats-row">
                <div className="stat-cell">
                  <div className={`stat-val cyan`}>{results ? results.total_faces : "—"}</div>
                  <div className="stat-label">Faces Found</div>
                </div>
                <div className="stat-cell">
                  <div className={`stat-val green`}>{results ? results.masked_count : "—"}</div>
                  <div className="stat-label">Masked</div>
                </div>
                <div className="stat-cell">
                  <div className={`stat-val red`}>{results ? results.unmasked_count : "—"}</div>
                  <div className="stat-label">Unmasked</div>
                </div>
              </div>
            </div>
            ...
            {results && (
              <div className="right-section">
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                  <div style={{ flex: 1 }}>
                    <div style={{ height: 8, background: "var(--border)", borderRadius: 1, overflow: "hidden" }}>
                      <div style={{
                        height: "100%",
                        width: `${results.compliance_rate}%`,
                        background: results.compliance_rate >= 80 ? "var(--green)" : results.compliance_rate >= 50 ? "var(--amber)" : "var(--red)",
                        transition: "width 0.8s ease",
                        boxShadow: results.compliance_rate >= 80 ? "0 0 8px rgba(0,255,136,0.5)" : ""
                      }} />
                    </div>
                    <div style={{ fontFamily: "var(--mono)", fontSize: "0.62rem", color: "var(--muted)", marginTop: 6 }}>
                      {results.compliance_rate >= 80 ? "✓ HIGH COMPLIANCE" : results.compliance_rate >= 50 ? "⚠ MODERATE — ACTION NEEDED" : "✕ LOW COMPLIANCE — ALERT"}
                    </div>
                  </div>
                  <div style={{
                    fontSize: "2rem", fontWeight: 700, minWidth: 64,
                    color: results.compliance_rate >= 80 ? "var(--green)" : results.compliance_rate >= 50 ? "var(--amber)" : "var(--red)"
                  }}>
                    {results.compliance_rate}%
                  </div>
                </div>
                {results.partial_count > 0 && (
                  <div style={{ marginTop: 10, fontFamily: "var(--mono)", fontSize: "0.65rem", color: "var(--amber)", padding: "6px 10px", border: "1px solid rgba(255,184,0,0.3)", background: "rgba(255,184,0,0.05)" }}>
                    ⚠ {results.partial_count} face(s) wearing mask incorrectly
                  </div>
                )}
              </div>
            )}
            ...
            <div className="right-section" style={{ flex: 1 }}>
              ...
              {!results && !loading && (
                <div className="empty-state">
                  ...
                  <div className="empty-text">
                    Upload an image and click<br />Analyze to begin scanning.<br /><br />
                    The AI will detect all faces<br />and classify mask compliance.
                  </div>
                </div>
              )}
              ...
              {loading && (
                <div className="loading-state">
                  <div className="radar" />
                  <div className="loading-text">SCANNING IMAGE...</div>
                  <div style={{ fontFamily: "var(--mono)", fontSize: "0.62rem", color: "var(--muted)", textAlign: "center", lineHeight: 1.8 }}>
                    Detecting faces<br />Classifying mask status<br />Computing compliance rate
                  </div>
                </div>
              )}

              {results && results.total_faces === 0 && (
                <div className="empty-state">
                  <div className="empty-icon">🚫</div>
                  <div className="empty-text">No faces detected in this image.<br />Try uploading a photo with visible people.</div>
                </div>
              )}

              {results && results.detections && (
                <div className="detection-list">
                  {results.detections.map((det, i) => <DetCard key={i} det={det} idx={i} />)}
                </div>
              )}
            </div>

            <div className="info-strip">
              <span>INTERNSHIP PROJECT · AI COMPUTER VISION</span>
              <span>Claude Vision · claude-sonnet-4</span>
            </div>
          </div>
        </div>
      </div>
